...
The current API has **407 unique tools**. Permission tags below describe the route's initial authority; shared operations also enforce workspace, resource, source-access and instance-admin requirements. Self-hosted tools are unavailable on the hosted Cloud controller.
...
| Tool and HTTP route | Description | Access |
| --- | --- | --- |
| `delete_wildcard_domains_by_id`<br />`DELETE /api/wildcard-domains/:id` | Delete a wildcard apex domain. | `domain:admin` |
| `get_wildcard_domains`<br />`GET /api/wildcard-domains` | List all registered wildcard apex domains. | `domain:list` |
| `get_wildcard_domains_by_id`<br />`GET /api/wildcard-domains/:id` | Get a wildcard apex domain by id. | `domain:read` |
| `patch_wildcard_domains_by_id`<br />`PATCH /api/wildcard-domains/:id` | Update a wildcard apex domain. | `domain:write` |
| `post_wildcard_domains`<br />`POST /api/wildcard-domains` | Register a new wildcard apex domain. | `domain:write` |
